    ...
    <div class="max-w-4xl mx-auto px-6 py-12">
        <div class="mb-8">
            <a href="{{ route('admin.posts.index') }}"
                class="text-gray-400 hover:text-white transition flex items-center gap-2 mb-4">
            ...
            <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <select name="category_id" id="category_id" required
                        ...
                        <option value="" class="bg-darkbg text-white">Select Category...</option>
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" class="bg-darkbg text-white" {{ old('category_id', $post->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                ...
                <div x-data="{
                    open: false,
                    search: '',
                    selected: {{ json_encode(collect(old('tags', $post->tags->pluck('id')->toArray()))->map(fn($i) => (int) $i)->values()) }},
                    ...
                    <div @click="open = true; $nextTick(() => $refs.searchInput.focus())" @click.away="open = false"
                        ...
                        <template x-for="item in selectedItems" :key="item.id">
                            <span
                                class="bg-accent/20 text-accent border border-accent/50 rounded-full px-3 py-1 text-sm flex items-center gap-1">
                    ...
                    <!-- Dropdown -->
                    <div x-show="open && filteredItems.length > 0" x-transition.origin.top
                    ...
                    <div x-show="open && filteredItems.length === 0"
                        class="absolute z-10 w-full bg-darkcard border border-gray-700 rounded-lg mt-1 shadow-xl p-4 text-gray-500 text-center text-sm">
                        No tags found.
                    </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        class="w-full bg-darkbg border border-gray-700 text-white rounded-lg px-4 py-3 focus:outline-none focus:border-accent transition">
                        <option value="draft" class="bg-darkbg text-white" {{ old('status', $post->status) == 'draft' ? 'selected' : '' }}>Draft
                        </option>
                        <option value="published" class="bg-darkbg text-white" {{ old('status', $post->status) == 'published' ? 'selected' : '' }}>
                            Published</option>
                    </select>
                ...
                <div class="pt-4 flex justify-end">
                    <button type="submit"
                        class="px-8 py-3 bg-accent text-white font-bold rounded-lg hover:bg-accent-hover transition transform hover:scale-105">
                        Update Post
                    </button>
                </div>
            </form>
